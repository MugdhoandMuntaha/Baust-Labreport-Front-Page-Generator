...
function getFilename(data: FormData, ext: string): string {
    const name = data.studentName || "LabReport";
    const sanitized = name.replace(/[^a-zA-Z0-9 ]/g, "").replace(/\s+/g, "_");
    const suffix = data.experimentNo ? `_Exp${data.experimentNo}` : "";
    return `${sanitized}${suffix}_LabReport.${ext}`;
}
...
export async function downloadDOCX(data: FormData, reportContent?: ReportContent | null): Promise<void> {
    ...
    const courseInfoTable = new Table({
        ...
        rows: [
            ...
            makeInfoRow("Experiment No", data.experimentNo),
            makeInfoRow("Experiment Name", data.experimentName),
        ],
    });
                    ...
                    new TableCell({
                        ...
                        children: [
                            ...
                            new Paragraph({
                                spacing: { after: 40 },
                                children: [
                                    new TextRun({ text: "Date of Experiment: ", bold: true, italics: true, size: 22, font: "Times New Roman" }),
                                    new TextRun({ text: data.experimentDate ? formatDate(data.experimentDate) : "", size: 22, font: "Times New Roman" }),
                                ],
                            }),
                            new Paragraph({
                                children: [
